...
- Can we predict `Income` using `Years of Education`? Can we predict `Income` using `Seniority`? Perhaps we can do better using a model using both?
$$
\text{Income} \approx f(\text{Education}, \text{Seniority})
$$

- $Y$: `Income` is the response or target that we wish to predict.

- $X$: `Education` and `Seniority`. 
$$
X = \begin{pmatrix} X_1 \\ \vdots \\ X_p \end{pmatrix}
$$
is the vector of features, or inputs, or predictor.
...
- Prediction. With a good $f$, we can predict $Y$ at new points $X = x$
$$
\hat Y = \hat f (X).
$$
- Inference. 
    ...
    - Depending on the complexity of $f$, we may be able to understand how each component $X_j$ of $X$ affects $Y$. For example, how much extra income will one earn with one more year of education? Does a linear relationship hold? 
...
- Even if we knew the truth $f$, we would still make errors in prediction due to the irreducbile error. 

## How to estimate $f$? 

Our goal is to apply a statistical learning method to find a function $\hat f$ such that $Y \approx \hat f(X)$ for any observation $(X, Y)$. 
...
![Yellow surface is the linear model fit $\hat f$](ISL_fig_2_4.pdf)
...
- Let $(x_0, y_0)$ be a test observation. Under the model @eq-statistical-model,
$$
...
- When the outcome $Y$ is discrete, for example, email is one of $\mathcal{C}=$\{`spam`, `ham`\} (`ham`=good email), digit class is one of $\mathcal{C} = \{0,1,\ldots,9\}$. 

- Our goals are to
    - build a classifer $f(X)$ that assigns a class label from $\mathcal{C}$ to a future unlabeled observation $X$;
...
$$
where $\hat y_i$ is the predicted class label for the $i$th observation using $\hat f$. As in the regression setting, we are most interested in the **test error rate** associated with a set of test obervations $(x_0, y_0)$
$$
\operatorname{Ave}(I(y_0 \ne \hat y_0))
$$
    
- Suppose $\mathcal{C}=\{1,2,\ldots,K\}$, the **Bayes classifer** assigns a test observation with predictor vector $x_0$ to the class $j$ for which
$$
...
- One simpple classifer is the **$K$-nearest neighbor (KNN)** classifier. Given a positive integer $K$ and a test observation $x_0$, the KNN classifer first identifies the $K$ points in the training data that are closest to $x_0$, represented by $\mathcal{N}_0$. It then estimates the conditional probability by
$$
\operatorname{Pr}(Y=j \mid X = x_0) = \frac{1}{K} \sum_{i \in \mathcal{N}_0} I(y_j = j)
$$
