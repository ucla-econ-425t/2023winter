...
- Inference. 
    ...
    - Causal inference. We may infer whether purposedly changing the values of certain variables will change outcomes. 

## The optimal (but unreachable) $f$
...
- However we almost never know the conditional distribution of $Y$ given $X$ in practice.
...
- Above U-shaped test MSE curves are in the so-called **classical regime** where number of features is less than the training samples. In the **modern regime** where with the number features can be order of magnitude larger than the training samples, the **double descent** phenomenon is observed and being actively studied. See this [paper](https://epubs.siam.org/doi/pdf/10.1137/20M1336072) and references therein.
...
- As in the regression setting, we are most interested in the **test error rate** associated with a set of test observations $(x_0, y_0)$
$$
