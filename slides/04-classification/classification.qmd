...
Default['x_student'] = Default['student'] == 'Yes'
y, X = Default['default'], Default[['x_student', 'balance', 'income']]

# Fit logistic regression
logit_fit = LogisticRegression().fit(X, y)
logit_fit
...
logit_mod <- glm(default ~ balance + income + student, family = binomial, data = Default)
logit_mod
...
- Another approach for classification is to model the distribution of $X$ in each of
the classes separately, and then use **Bayes theorem** to flip things around and obtain $\operatorname{Pr}(Y = j \mid X)$.
...
<p align="center">
![](ISL_fig_4_6.pdf){width=500px height=300px}
</p>
...
lda_cfm
```
Overall training accuracy of LDA is
```{python}
...
# Fit LDA
lda_mod <- lda(default ~ balance + income + student, data = Default)
lda_mod
# Predicted labels from LDA
lda_pred = predict(lda_mod, Default)
# Confusion matrix
table(Predicted = lda_pred$class, Default = Default$default)
```
...
<p align="center">
![](ISL_fig_4_9.pdf){width=500px height=300px}
</p>
...
qda_cfm
```
Overall training accuracy of QDA is
```{python}
...
```{r}
# Fit QDA
qda_mod <- qda(default ~ balance + income + student, data = Default)
qda_mod
# Predicted probabilities from QDA
qda_pred = predict(qda_mod, Default)
# Confusion matrix
table(Predicted = qda_pred$class, Default = Default$default)
```
...
nb_cfm
```
Overall training accuracy of Naive Bayes classifier is
```{python}
...
nb_mod <- naiveBayes(default ~ balance + income + student, data = Default)
nb_mod
# Predicted labels from Naive Bayes
nb_pred = predict(nb_mod, Default)
# Confusion matrix
table(Predicted = nb_pred, Default = Default$default)
```
...
# Fit KNN with K = 5
knn_fit = KNeighborsClassifier(n_neighbors = 5).fit(X, y)
...
knn_cfm
```
Overall training accuracy of KNN classifier with $K=5$ is
```{python}
...
- Let's summarize the classification performance of diffiferent classifiers on the training data. 
...
for m in classifiers:
    roc_display = RocCurveDisplay.from_estimator(m, X, y)
    plt.show()
```
