...
- In particular, at each step the variable that gives the greatest additional improvement to the fit is added to the
model.
...
- Indirect approaches.
    ...
    $$
    where $d$ is the total number of parameters used and $\hat{\sigma}^2$ is an estimate of the variance of error $\epsilon$. 
...
- Direct approaches: validation and cross-validation. We'll discuss in details in the next lecture. 
...
- Recall that the least squares fitting procedure estimates $\beta_0, \beta_1, \ldots, \beta_p$ using the value that minimizes
$$
...
Simulated data with $n = 50$ observations, $p = 45$ predictors, all having nonzero coefficients. Squared bias (black), variance (green), and test mean squared error (purple) for the ridge regression predictions on a simulated data set, as a function of $\lambda$ and $\|\hat{\beta}_{\lambda}^R\| / \|\hat{\beta}\|_2$. The horizontal dashed lines indicate the minimum possible MSE. The purple crosses indicate the ridge regression models for which the MSE is smallest.
...
- As with ridge regression, the lasso shrinks the coecient
...
One can show that the lasso and ridge regression coecient
