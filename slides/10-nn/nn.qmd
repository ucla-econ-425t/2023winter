---
title: "Neural Network and Deep Learning (ISL 10)"
subtitle: "Econ 425T"
...
is high, e.g., image recognition and language translation. Datasets are large, and overfitting is not a big problem.
...
$$
R_i(\theta) = \frac 12 (y_i - f_\theta(x_i))^2 = \frac 12 (y_i - \beta_0 - \sum_{k=1}^K \beta_k g(w_{k0} + \sum_{j=1}^p w_{ij} w_{ij}))^2.
$$
For ease of notation, let
$$
z_{ik} = w_{k0} + \sum_{j=1}^p w_{ij} w_{ij}.
$$
...
- Tricks of the trade
    ...
    - An **epoch** is a count of iterations and amounts to the number of minibatch updates such that $n$ samples in total have been processed; i.e. $60,000/128 \approx 469$ for `MNIST`.
...
- Make many copies of each $(xi, yi)$ and add a small amount of Gaussian noise to the $x_i$, a little cloud around each
...
- $y= sin(x) + \epsilon$ with $x \sim Uniform(-5, 5)$ and $\epsilon$ Gaussian with s.d.=0.3.
...
above 20, $\sum_{j=1}^d \hat \beta_j^2$ **decreases** since it is easier to achieve zero error, and hence less wiggly solutions.
